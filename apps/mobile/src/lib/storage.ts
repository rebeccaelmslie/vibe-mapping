...
const EMPTY_ANNOTATIONS: MapAnnotations = { pins: [] };

export function getAnnotations(token: string): Promise<MapAnnotations> {
  return readJSON<MapAnnotations>(annotationsKey(token), EMPTY_ANNOTATIONS);
}
